...
import {
  Upload, FileText, Sliders, BarChart3, Building2, Plus, ArrowRight,
  Download, RefreshCw, AlertCircle, Check, X, Trash2, Eye, ChevronRight,
  TrendingDown, Zap, Shield, AlertTriangle, Database, Settings, FileDown,
  Layers, Activity, DollarSign, Users, Calendar, MapPin, Target, Edit3,
} from "lucide-react";
...
function generateSyntheticClaims(coveredLives, annualSpend) {
  ...
  SYNTHETIC_DISTRIBUTION.forEach(([category, bucket, share, avgSize, cpt, pos]) => {
    ...
    const claimCount = Math.max(1, Math.round(categorySpend / avgSize));
    ...
    for (let i = 0; i < claimCount; i++) {
      ...
      const amount = Math.round(avgSize * variance);
...
export default function ClaimsReclassificationEngine() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, system-ui, sans-serif" }}
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Instrument+Serif:ital@0;1&family=JetBrains+Mono:wght@400;500;600&family=Inter:wght@400;500;600;700&display=swap');
        ...
        {screen === SCREENS.UPLOAD && (
          <UploadScreen
            ...
            onSyntheticGenerate={async (lives, spend) => {
              const synth = generateSyntheticClaims(lives, spend);
              ...
              await storage.set(`input_mode:${activeEmployerId}`, inputModeRow);
              await saveClaims(classified);
              showToast(`Modeled dataset built · ${classified.length} synthetic lines`, "success");
              setScreen(SCREENS.CLASSIFY);
            }}
            showToast={showToast}
          />
...
function ClassifyScreen({ claims, onUpdateClaim }) {
  ...
  const filtered = useMemo(() => {
    return claims.filter((c) => {
      ...
      if (filter && !JSON.stringify(c).toLowerCase().includes(filter.toLowerCase())) return false;
      return true;
    });
