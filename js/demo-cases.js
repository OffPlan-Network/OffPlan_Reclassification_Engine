/* =====================================================================
 *  OFFPLAN RECLASSIFICATION ENGINE — DEMO CASES
 *  ---------------------------------------------------------------------
 ...
 *                            (Full Claims). Demonstrates the Mode 1 flow
 *                            with a realistic-volume dataset; flagged in
 *                            provenance as benchmark-derived.
 *    - "csv_partial":       fetch a partial-summary CSV from /data/ and
 *                            run it through the Mode 2 ingestion path.
 *    - "rows_partial":      inline partial-summary rows (same shape as
 *                            CSV columns) decomposed via the Mode 2 path.
 *                            Useful when a demo needs custom totals that
 *                            don't match any bundled CSV.
 *    - "modeled":           use the synthetic generator and stamp
 *                            Mode 3 / low confidence per spec.
 * ===================================================================== */
...
(function () {
  const DEMO_CASES = [
    ...
    },
    {
      ...
      // Construction-flavored category totals. Heavy on ER (jobsite injuries)
      // and Procedures (orthopedic), light on Inpatient. Total = $540K to
      // match historical_claims_spend.
      loader: {
        kind: "rows_partial",
        rows: [
  ...
  window.OFFPLAN_DEMO = { DEMO_CASES, SAMPLE_CSV_FILES };
})();
