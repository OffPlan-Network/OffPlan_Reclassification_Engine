...
- **Specialty Rx monthly-recurrence regimen (T10)** — per Liquidity Spec v1.2 §4. T10 has `regimen_mode: 'monthly_recurrence'` in `EVENT_TIER_CATALOG`. At run start, `regimen_member_fraction × lives = 3%` of population is sampled from the chronic pool as "on regimen"; each regimen member generates `fills_per_member_year = 12` monthly events with cost sampled per fill from the log-normal. This bypasses the standard Poisson sampling and chronic-uplift pathway for T10 (T10 is deliberately omitted from `CHRONIC_TIER_UPLIFT`). Produces realistic ~$80–$105 PMPM specialty-Rx exposure concentrated on a small subset, mirroring biologics/specialty-Rx cost concentration in real claims data.
- **Bimodal Maternity / NICU (T11 + T12)** — per Liquidity Spec v1.2 §4. Replaces the prior single log-normal with a routine + complicated split: T11 (routine delivery) at λ=0.008, Bucket B (cash-pay repriced like outpatient specialty surgery — plannable care priced at contracted rates), mean $12K. T12 (NICU complications) at λ=0.002, Bucket E (catastrophic, stop-loss eligible), log-normal σ=0.7 mean $80K with the heavy NICU tail. 80/20 routine/complicated split anchored to CDC/NCHS births data. Both trigger Hospital Admission indemnity offset.
